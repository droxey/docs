...
module.exports.uglify = function(callback) {
  ...
  var materialCssfileContents = fs.readFileSync(
    path.join("public", "stylesheets", "material-style.css"),
    "utf8"
  );
  var materialCssUglified = uglifycss.processString(materialCssfileContents);
  fs.writeFileSync(
    path.join("public", "stylesheets", "material-style.min.css"),
    materialCssUglified,
    "utf8"
  );
...
module.exports.build = function(callback) {
  if (typeof config.docs !== "undefined" && config.docs.length > 0) {
    fs.emptyDir("docs")
      .then(() => {
        config.docs.forEach(function(doc) {
          ...
          fs.writeFileSync(
            path.join("docs", doc.title.toLowerCase() + ".html"),
    ...
    console.log(
      "[ERROR] No docs to process. Add some to your config.json file"
    );
  }
};
